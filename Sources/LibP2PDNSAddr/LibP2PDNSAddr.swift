...
//
//
//  Created by Brandon Toms on 1/20/22.
//
...
public final class DNSAddr: AddressResolver {
    ...
    public func resolve(multiaddr: Multiaddr, for codecs:Set<MultiaddrProtocol> = [.ip4, .tcp]) -> EventLoopFuture<Multiaddr?> {
    ...
    static func resolve(address ma:Multiaddr, for codecs:Set<MultiaddrProtocol> = [.ip4, .tcp], callback:@escaping (Multiaddr?) -> Void) {
        return callback(resolve(address: ma, for: codecs))
    }
    ...
    static func resolve(address ma:Multiaddr, for codecs:Set<MultiaddrProtocol> = [.ip4, .tcp]) -> Multiaddr? {
        ...
        return addresses.first(where: { addy in
            return Set(addy.addresses.map { $0.codec }).isSuperset(of: codecs)
        })
    }
    ...
    static func resolve(address ma:Multiaddr) -> [Multiaddr]? {
        ...
        guard ma.addresses.first?.codec == .dnsaddr, let domain = ma.addresses.first?.addr, let pid = ma.getPeerID() else {
            return nil
        }
        ...
        guard let host = firstResolution?.first(where: { key, val in
            key.contains(pid)
        }) else {
        ...
        guard hostMA.addresses.first?.codec == .dnsaddr, let domain2 = hostMA.addresses.first?.addr, let pid2 = hostMA.getPeerID(), pid == pid2 else {
            return [hostMA]
        }
...
extension Multiaddr {
    ...
    public func resolve(for codecs:Set<MultiaddrProtocol> = [.ip4, .tcp]) -> Multiaddr? {
        return DNSAddr.resolve(address: self, for: codecs)
    }
    
    public func resolve(address ma:Multiaddr, for codecs:Set<MultiaddrProtocol> = [.ip4, .tcp], callback:@escaping (Multiaddr?) -> Void) {
        return DNSAddr.resolve(address: self, for: codecs, callback: callback)
    }
